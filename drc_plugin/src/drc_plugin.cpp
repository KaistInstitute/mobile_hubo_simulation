...
//    int threadID_World = pthread_create(&WORLDTHREAD_t, NULL, &WorldThread, this);
//    if(threadID_World < 0){
//        std::cout << "Create World Thread Error.." << std::endl;
//    }
...
std::cout << "End Run" << std::endl;
        usleep(50);
//        while(1){
//            //if(dp->world->IsPaused()){
//            //if(dp->world->GetRunning() == false){
//            if(dp->world->GetRunning() == false){
//                //std::cout << "is paused" << std::endl;
//                break;
//            }
//            usleep(5);
//            //std::cout << "GetRunning: " << dp->world->GetRunning() << std::endl;
//        }
std::cout << "End GetRunning" << std::endl;


        // RWFT, LWFT ------
//        physics::JointWrench wrenchR = dp->JPtr_RWFT->GetForceTorque(0);
//        math::Vector3 RWF = wrenchR.body1Force;
//        math::Vector3 RWT = wrenchR.body1Torque;
//        dp->filt_RWF = dp->filt_alpha*RWF + (1-dp->filt_alpha)*dp->filt_RWF;
//        dp->filt_RWT = dp->filt_alpha*RWT + (1-dp->filt_alpha)*dp->filt_RWT;
//        if(dp->connectionStatus){
//            dp->TXSensorData.FTSensor[2].force[0]   = dp->filt_RWF.x;
//            dp->TXSensorData.FTSensor[2].force[1]   = dp->filt_RWF.y;
//            dp->TXSensorData.FTSensor[2].force[2]   = dp->filt_RWF.z;
//            dp->TXSensorData.FTSensor[2].torque[0]  = dp->filt_RWT.x;
//            dp->TXSensorData.FTSensor[2].torque[1]  = dp->filt_RWT.y;
//            dp->TXSensorData.FTSensor[2].torque[2]  = dp->filt_RWT.z;
//        }
//        physics::JointWrench wrenchL = dp->JPtr_LWFT->GetForceTorque(0);
//        math::Vector3 LWF = wrenchL.body1Force;
//        math::Vector3 LWT = wrenchL.body1Torque;
//        dp->filt_LWF = dp->filt_alpha*LWF + (1-dp->filt_alpha)*dp->filt_LWF;
//        dp->filt_LWT = dp->filt_alpha*LWT + (1-dp->filt_alpha)*dp->filt_LWT;
//        if(dp->connectionStatus){
//            dp->TXSensorData.FTSensor[3].force[0]   = dp->filt_LWF.x;
//            dp->TXSensorData.FTSensor[3].force[1]   = dp->filt_LWF.y;
//            dp->TXSensorData.FTSensor[3].force[2]   = dp->filt_LWF.z;
//            dp->TXSensorData.FTSensor[3].torque[0]  = dp->filt_LWT.x;
//            dp->TXSensorData.FTSensor[3].torque[1]  = dp->filt_LWT.y;
//            dp->TXSensorData.FTSensor[3].torque[2]  = dp->filt_LWT.z;
//        }
